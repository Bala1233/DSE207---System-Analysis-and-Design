$(function () {
    var ProductId = document.URL.split("id=")[1];
    $.ajax({
        type: "POST",
        url: "/CustomerStoreFunction/GetProductDetails?id=" + ProductId,
        success: function (data) {

            $("#ProductDetails").append(`
                                        <div class="col-10" style = "margin-left:auto;margin-right:auto" >
                            <div style="padding-top:3rem;" class="row w-100 m-0 pb-5 border-bottom ">
                                <div class="slide col-md-6 p-0">
                                    <div class="slide__main h-75" style="display:flex;justify-content: center;">
                                        <img class="OnShowPic" style="width:50%;" src="${data.productImg1}" alt="">
                                    </div>
                                    <div style="padding-top:2rem;justify-content: center" class="slide__image row h-25 w-100 m-0 d-flex">
                                        <div class="col-2 mr-3 p-0">
                                            <img style="width:100%;" class="slide__image__child active smallPic" onerror="ErrorImg(this)" src="${data.productImg1}" alt="">
                                        </div>
                                        <div class="col-2 p-0 mr-3">
                                            <img style="width:100%;" class="slide__image__child smallPic" src="${data.productImg2}" alt="">
                                        </div>
                                        <div class="col-2 p-0 mr-3">
                                            <img style="width:100%;" class="slide__image__child smallPic" src="${data.detailsImg1}" alt="">
                                        </div>
                                        <div class="col-2 p-0 mr-3">
                                            <img style="width:100%;" class="slide__image__child smallPic" src="${data.detailsImg2}" alt="">
                                        </div>
                                        <div class="col-2 p-0">
                                            <img style="width:100%;" class="slide__image__child smallPic" onerror="ErrorImg(this)" src="${data.detailsImg3}" alt="">
                                        </div>
                                    </div>
                                </div>

                                <div class="detail col-md-6 pr-0">
                                    <div class="info pb-3 border-bottom ">
                                        <h4 id="ProductName">${data.productName}</h4>
                                        <span class="text-secondary">${data.title}</span>
                                        <h3>$ ${data.disPrice}   </h3>
                                    </div>
                                    <input type="hidden" id="ProId" value="${data.productId}" />
                                    <div class="pt-3 pb-3 border-bottom d-flex">
                                            ...
                                            <input type="hidden" name="form_type" value="product"><input type="hidden" name="utf8" value="✓">
                                                ...
                                                <div class="product-form__item product-form__item--quantity">
                                                    ...
                                                    <div class="js-qty">
                                                        ...
                                                        <input oninput="javascript: if(this.value <=0)  this.value=1" type="number" value="1" id="Quantity" name="quantity" pattern="[0-9]*" data-line="" class="js-qty__input" aria-live="polite" mm-min="1" mm-stock-max="NaN" step="1" mm-step="1">
                                                            <!--MinusBtn-->
                                                            <button id="Minusbtn" type="button" class="js-qty__adjust js-qty__adjust--minus" aria-label="Reduce item quantity by one">
                                                            ...
                                                            <!--AddBtn-->
                                                            <button id="Addbtn" value="${data.qty}"type="button" class="js-qty__adjust js-qty__adjust--plus" aria-label="Increase item quantity by one">
                                                ...
                                                <div class="product-form__item product-form__item--submit">
                                                    <button type="submit" name="add" id="AddToCart-product-template" class="btn btn--full product-form__cart-submit btn--secondary-accent AddToCart">
    ...
    <!-- <div class="pt-3 pb-3 d-flex">
       <button type="button" onclick="AddtoCart()" class="btn btn-primary AddToCart">ADD TO CART</button>
                                    ...
                                    <div style="justify-content: end;" class="pt-3 pb-3 d-flex">
                                        <button type="button" onclick="BackToHome()" class="btn btn-outline-secondary">Back To Home Page</button>
                                    </div>
                                </div>
                                <div class="col-1"></div>
            ...
            $("#Quantity").keyup(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "/CustomerStoreFunction/GetProductStock?productId=" + productId,
                    success: function (data) {
                        if (inputValue > data) {
                            $("#Quantity").val(data)
            ...
            $(".AddToCart").click(function () {
                ...
                var productId = $("#ProId").val();
                var InputQty = $("#Quantity").val()
                var profuctname = $("#ProductName").html()

                $.ajax({
                    type: "Get",
                    url: `/CustomerStoreFunction/AddtocartAjax?ProductId=${PID}&Qty=${Qty}`,
                    success: function (data) {
                        if (data == "Please Login") {
                            $(".Remin").show(500, function () {
                                setTimeout(function () {
                                    location.href = "/CustomerLogin/LoginPage"
                                }, 1000)
                            })
                        }
                        ...
                        else if (data == "Your cart is full") {
                            $(".CartRemind").show()
                            $(".Addsuccess").hide()
                        }
                        else if (data == "Added Successful") {
                            $(".AddSuccess").show()
                            $(".OutofStock").hide()
                            $(".CartRemind").hide()
                            $.ajax({
        ...
        <div style="padding:8px">
            ...
            <input id="DiaplayValue" readonly style="font-size:18px;margin:10px;background-color:transparent;width:50px;color:white"
                   class="fontContent" value="${pair.cart.qty}" />
            <!-- Plus button -->
    ...
    <!-- end of each product row -->
                                `)
                                    ...
                                    $(".fa-minus").click(function () {
                                        ...
                                        $.ajax({
                                            ...
                                            data: { CartId: $(this).attr("data-CartId") },
                                            success: function (data) {
                                                $("#DiaplayValue").val(data)
            ...
            $("#Addbtn").click(function () {
                ...
                var InputQty = $("#Quantity").val()
                var qty = $(this).val()
                if (parseInt(InputQty) >= parseInt(qty)) {
                    return
                } else {
                    InputQty++
                    $("#Quantity").val(InputQty)
                }
                //var profuctname = $("#ProductName").html()
                ...
                //$.ajax({
                //    type: "POST",
                //    url: "/CustomerStoreFunction/ProductDetailsAddQty?productid=" + productId + "&qty=" + InputQty,
                //    success: function (data) {
                //        if (data == true) {
                //            input.prev().prev().val(parseInt(InputQty) + 1);
                //            $(".Error_Msg").text("")
                //        }
                //        else {
                //            $(".Error_Msg").text(`Can't Add More Items`)
                //        }
                //    }
                //})
            })
            ...
            $("#Minusbtn").click(function () {
                ...
                $(this).prev().val(parseInt(InputQty) - 1);
                $.ajax({
                    type: "POST",
                    url: "/CustomerStoreFunction/GetProductStock?productId=" + productId,
                    success: function (data) {
